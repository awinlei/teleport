...
func (b *DynamoDBBackend) GetRange(ctx context.Context, startKey []byte, endKey []byte, limit int) (*backend.GetResult, error) {
	...
	result, err := b.getRecords(ctx, trimPrefix(startKey), trimPrefix(endKey), limit)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (b *DynamoDBBackend) DeleteRange(ctx context.Context, startKey, endKey []byte) error {
	...
	for i := 0; i < backend.DefaultLargeLimit/100; i++ {
		result, err := b.getRecords(ctx, trimPrefix(startKey), trimPrefix(endKey), 100)
		if err != nil {
...
func (b *DynamoDBBackend) CompareAndSwap(ctx context.Context, expected backend.Item, replaceWith backend.Item) (*backend.Lease, error) {
	...
	r := record{
		HashKey:   hashKey,
		FullPath:  trimPrefix(replaceWith.Key),
...
func (b *DynamoDBBackend) KeepAlive(ctx context.Context, lease backend.Lease, expires time.Time) error {
	...
	input := &dynamodb.UpdateItemInput{
		...
		Key: map[string]*dynamodb.AttributeValue{
			...
			fullPathKey: {
				S: aws.String(trimPrefix(lease.Key)),
			},
		},
...
// trimPrefix removes leading '/' from the key for backwards compatibility
// with previous implementation of DynamoDB backend
func trimPrefix(key []byte) string {
	return string(bytes.TrimPrefix(key, []byte{backend.Separator}))
}

// addPrefix adds leading '/' to the key
func addPrefix(key string) []byte {
	return []byte(string(backend.Separator) + key)
}
